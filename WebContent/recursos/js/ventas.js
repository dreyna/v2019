...
$("#prod").keyup(function (){
    ...
    $.get("ventas",{"opc":1,"cod":cod},function(data) {
        ...
        if(data==0){
            ...
            clear();            
            if(x.stock >0){           
                $("#tablaVentas").append("<tr><td><input type='text' id='cant' value='1' onkeyup='validar("+x.stock+")' class='form-control' style='width: 50px;'" +
...
function validar(stock){
	if($("#cant").val()>stock || $("#cant").val()==0)
		alert("No exiten suficiente stock");
}
function imprimir() {
  window.print();
}
